...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        # Compute gradients
        output_layer_error = (activated_output - y) * output_layer_activation_derivative(output) # dev of loss * dev of output activation function
        hidden_layer_error = np.multiply(np.vectorize(rectified_linear_unit_derivative)(hidden_layer_weighted_input).T , self.hidden_to_output_weights).T * output_layer_error # 3 by 1 matrix

        bias_gradients = hidden_layer_error
        hidden_to_output_weight_gradients = (hidden_layer_activation * output_layer_error).T
