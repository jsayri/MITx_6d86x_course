...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    Note: each eigenvector is already be a unit-vector, so the
          projection may be done using matrix multiplication.
    """
    Xc = X - feature_means # data centered by using the feature_means
    # Return the projection of the centered dataset on the first n_components components.
    return np.matmul(pcs[:, 0:n_components].T, Xc.T).T # dimensions: n x n_components
