...
def run_softmax_on_MNIST(temp_parameter=1, error_mod3 = False):
    """
...
def run_pca_softmax_on_MNIST(train_pca, train_y, test_pca, test_y, temp_parameter=1):
    """
    Trains Softmax regression on digit classifications with pca conversion.
    It uses the following values for parameters:
    alpha = 0.3
    lambda = 1e-4
    num_iterations = 150
...
# Run run_pca_softmax_on_MNIST(), report the error rate
# print('softmax with pca test_error=', run_pca_softmax_on_MNIST(train_pca, train_y, test_pca, test_y, temp_parameter=1))
...
# The cubic train and test features with cubic_features function in features.py are define
...
def run_cubic_pca_softmax_on_MNIST(train_cube, train_y, test_cube, test_y, temp_parameter=1):
    """
    Trains Softmax regression on digit classifications with cubic pca conversion.
    It uses the following values for parameters:
    alpha = 0.3
    lambda = 1e-4
    num_iterations = 150
    ...
    # Save the model parameters theta obtained from calling softmax_regression to disk.
    write_pickle_data(theta, "./theta_cubic_pca.pkl.gz")
