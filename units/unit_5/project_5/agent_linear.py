...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.random() > (1 - epsilon):
        ...
        action_index, object_index = np.unravel_index((theta @ state_vector).argmax(), (NUM_ACTIONS, NUM_OBJECTS))
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    Returns:
        None
    """
    y = reward + int(not terminal) * GAMMA * (theta @ next_state_vector).max()
    q_sc = (theta @ current_state_vector)[tuple2index(action_index, object_index)]
    theta[tuple2index(action_index, object_index)] += ALPHA * (y - q_sc) * current_state_vector
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        # get action from epsilon strategy
